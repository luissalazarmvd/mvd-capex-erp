...
function displayNumber(value: unknown) {
  ...
  return parsed.toLocaleString("es-PE", { minimumFractionDigits: 2, maximumFractionDigits: 6 });
}

function toDraft(row: DeprRow): Draft {
  const draft = {} as Draft;
  EDITABLE.forEach((key) => { draft[key] = text(row[key]); });
  return draft;
}
...
export default function FixAssetsDepr() {
  ...
  }).sort((a, b) => text(a.asset_code).localeCompare(text(b.asset_code), undefined, { numeric: true })), [rows, year, month]);
    ...
    setDrafts((current) => {
      ...
      if (key === "applied_rate_pct") {
        if (!value.trim() || !validOptionalNumber(value)) {
          next.depreciation_amount_pen = value.trim() ? next.depreciation_amount_pen : "";
        } else {
          const calculated = num(row.asset_base_value) * (Number(value) / 12);
          const availableBeforePeriod = Math.max(
            0,
            num(row.asset_balance_pen)
              + num(originals[id]?.depreciation_amount_pen ?? row.depreciation_amount_pen)
          );
      ...
      if (key === "depreciation_amount_pen") {
        if (!value.trim() || !validOptionalNumber(value)) {
          ...
          const base = num(row.asset_base_value);
          next.applied_rate_pct = base ? precise((Number(value) * 12) / base) : "0";
        }
      }
      ...
      <div className="panel-inner" style={{ overflow: "auto", maxHeight: "calc(100vh - 260px)", padding: 0 }}>
        <div style={{ minWidth: "max-content" }}>
          <Table disableScrollWrapper>
            ...
            <thead><tr><th className="capex-th" style={{ padding: "8px", fontSize: 12, textAlign: "center" }}>Enviar</th>{COLUMNS.map((column) => <th key={column.key} className="capex-th" style={{ padding: "8px", fontSize: 12 }}>{column.label}</th>)}</tr></thead>
            <tbody>
              {visibleRows.map((row) => {
                const id = rowKey(row);
                ...
                return <tr key={id} className="capex-tr">
                  <td className="capex-td" style={{ padding: 5, textAlign: "center", background }}>
                    <input
                      type="checkbox"
                      checked={selected}
