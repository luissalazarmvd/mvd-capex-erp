// src/app/traceability/layout.tsx
import Image from "next/image";
import TraceabilityTopNav from "../../components/traceability/TraceabilityTopNav";
import LogoutLink from "../../components/auth/LogoutLink";

export default function TraceabilityLayout({ children }: { children: React.ReactNode }) {
  return (
    <div data-module="traceability" style={{ minHeight: "100vh", background: "var(--bg)" }}>
      <header
        style={{
          background: "var(--header)",
          borderBottom: "1px solid var(--border)",
        }}
      >
        <div className="container-fluid" style={{ paddingTop: 14, paddingBottom: 14 }}>
          <div
            style={{
              display: "flex",
              alignItems: "center",
              justifyContent: "space-between",
              gap: 14,
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <LogoutLink
                aria-label="Inicio"
                title="Inicio"
                style={{
                  width: 130,
                  height: 44,
                  position: "relative",
                  display: "block",
                  textDecoration: "none",
                }}
              >
                <Image
                  src="/logo_mvd.png"
                  alt="MVD"
                  fill
                  priority
                  style={{ objectFit: "contain" }}
                  sizes="130px"
                />
              </LogoutLink>

              <div style={{ lineHeight: 1.1 }}>
                <div style={{ fontSize: 19, fontWeight: 500, letterSpacing: "-.01em" }}>MVD – TRAZABILIDAD</div>
                <div className="muted" style={{ fontSize: 12 }}>
                  Datos Valorización · Validar Datos · Mineral No Disponible · Lotes Pagados · CM Inputs
                </div>
              </div>
            </div>

            <LogoutLink
              className="nav-pill !text-white visited:!text-white text-lg font-extrabold"
            >
              Inicio
            </LogoutLink>
          </div>

          <div style={{ marginTop: 12 }}>
            <TraceabilityTopNav />
          </div>
        </div>
      </header>

      <main className="container-fluid" style={{ paddingTop: 16, paddingBottom: 30 }}>
        <section
          className="panel"
          style={{
            padding: 14,
            minHeight: "calc(100vh - 160px)",
          }}
        >
          {children}
        </section>
      </main>
    </div>
  );
}
