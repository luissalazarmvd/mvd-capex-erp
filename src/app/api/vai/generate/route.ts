...
- PEN y USD siempre van en gráficos separados. Las comparaciones por sede, área, proveedor, responsable, placa, conductor, oficina, estado o cualquier otra dimensión válida no tienen restricciones adicionales.
...
async function generateDashboardSpec(
  ...
  const apiKey =
    process.env.API_OPEN_AI?.trim();

  if (!apiKey) {
    throw new VaiGenerationError(
      "V-Ai no está configurado en este entorno.",
      "missing API_OPEN_AI",
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new VaiGenerationError(
      "El servicio de IA devolvió un error. Inténtalo de nuevo en unos minutos.",
  ...
  if (!output) {
    throw new VaiGenerationError(
      ...
      `unparseable output: ${String(text).slice(0, 300)}`,
    );
  }
