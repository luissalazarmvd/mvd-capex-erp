...
const PortfolioYearContext = createContext<PortfolioYear>(2026);
...
function resultForYear(result: ProjectResult, year: PortfolioYear, lang: Lang): ProjectResult {
  if (year === 2026) return result;
  const baselineMh = Number.isFinite(result.avgCurrent) ? result.avgCurrent : 0;
  const currentUsdRows = result.rows.filter((row) => Number.isFinite(row.currentUsd)).map((row) => Number(row.currentUsd));
  const baselineUsd = Number.isFinite(mean(currentUsdRows)) ? mean(currentUsdRows) : 0;
  const rows: MonthlyRow[] = Array.from({ length: 12 }, (_, index) => ({
    month: `2027-${String(index + 1).padStart(2, "0")}`,
    period: tr(lang, "New baseline", "Nouvelle référence"),
    workload: tr(lang, "Optimized 2026 state carried forward", "État optimisé de 2026 reporté"),
    beforeMh: baselineMh,
    beforeUsd: baselineUsd,
    currentMh: baselineMh,
  ...
  return {
    ...
    totalUsd: 0,
    rows,
    chart: rows.map((row) => ({ month: row.month, legacy: baselineMh, current: baselineMh })),
    extraLabel: tr(lang, "Baseline year", "Année de référence"),
    extraValue: "2027",
    extraSub: tr(lang, "Optimized 2026 state carried forward", "État optimisé de 2026 reporté"),
  };
}
...
function MonthlyTable({ rows, lang, showTotals = false }: { rows: MonthlyRow[]; lang: Lang; showTotals?: boolean }) {
  const totals = {
    workload: sum(rows.map((row) => row.workloadCount || 0)),
    beforeMh: sum(rows.map((row) => row.beforeMh)),
    beforeUsd: sum(rows.map((row) => row.beforeUsd)),
    currentMh: sum(rows.filter((row) => row.currentMh !== null).map((row) => Number(row.currentMh))),
    currentUsd: sum(rows.filter((row) => row.currentUsd !== null).map((row) => Number(row.currentUsd))),
    savedMh: sum(rows.filter((row) => row.savedMh !== null).map((row) => Number(row.savedMh))),
    savedUsd: sum(rows.filter((row) => row.savedUsd !== null).map((row) => Number(row.savedUsd))),
  };
  return (
    <div className="ti-table-box">
      ...
      <div className="ti-table-wrap">
        <table>
          ...
          {showTotals ? <tfoot><tr className="ti-sum-row">
            <td>{tr(lang, "Total", "Total")}</td><td>—</td><td>{number(totals.workload)} {tr(lang, "records", "enregistrements")}</td>
            <td>{number(totals.beforeMh, 2)}</td><td>{money(totals.beforeUsd)}</td><td>{number(totals.currentMh, 2)}</td>
            <td>{money(totals.currentUsd)}</td><td>{number(totals.savedMh, 2)}</td><td>{money(totals.savedUsd)}</td>
          </tr></tfoot> : null}
        </table>
      </div>
    </div>
...
function ProjectCard({
  ...
  const displayed = resultForYear(result, year, lang);
  return (
    <details className="ti-project" open={open} onToggle={(event) => onToggle(event.currentTarget.open)}>
      <summary>
        ...
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Before · MH/month", "Avant · HP/mois")}</div><div className="ti-summary-value">{number(displayed.avgBefore, 2)} {tr(lang, "MH/month", "HP/mois")}</div></div>
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "After · MH/month", "Après · HP/mois")}</div><div className="ti-summary-value">{number(displayed.avgCurrent, 2)} {tr(lang, "MH/month", "HP/mois")}</div></div>
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Optimization", "Optimisation")}</div><div className="ti-summary-value good">{percent(displayed.optimization)}</div></div>
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Annualized USD savings", "Économies USD annualisées")}</div><div className="ti-summary-value">{money(displayed.avgUsd * 12)}</div></div>
      ...
      <div className="ti-project-body">
        ...
        {year === 2026 ? controls : <div className="ti-year-note"><strong>2027</strong><span>{tr(lang, "The optimized 2026 result is now the operating baseline. Incremental savings restart at zero.", "Le résultat optimisé de 2026 devient la référence opérationnelle. Les économies incrémentales repartent de zéro.")}</span></div>}
        <Kpis result={displayed} lang={lang} />
        <Comparison result={displayed} note={year === 2027 ? tr(lang, "2027 starts from the optimized 2026 baseline; no additional initiative has been applied yet.", "2027 démarre sur la référence optimisée de 2026 ; aucune initiative supplémentaire n’a encore été appliquée.") : note} lang={lang} />
        {year === 2026 ? extra : null}
        <MonthlyChart points={displayed.chart} title={chartTitle} lang={lang} />
        <MonthlyTable rows={displayed.rows} lang={lang} showTotals={year === 2026 && showTotals} />
        <div className="ti-method"><strong>{tr(lang, "Method.", "Méthode.")}</strong> {year === 2027 ? tr(lang, "The 2026 optimized version is carried into 2027 as both the before and after state. Therefore optimization and incremental savings are zero until a new 2027 improvement is registered.", "La version optimisée de 2026 est reportée en 2027 comme état avant et après. L’optimisation et les économies incrémentales sont donc nulles jusqu’à l’enregistrement d’une nouvelle amélioration en 2027.") : method}</div>
      </div>
    </details>
  );
}
...
function FixedAssetsProject({ rows, open, onToggle, onResult, lang }: { rows: FixedAssetFinanceRow[]; open: boolean; onToggle: (open: boolean) => void; onResult: (metric: PortfolioMetric) => void; lang: Lang }) {
  ...
  const result = useMemo(() => {
    ...
    monthsFrom(from, to).forEach((month) => {
      ...
      monthly.push(isBefore ? {
        ...
        workloadCount: rowCount,
        beforeMh: legacyMh,
        beforeUsd: legacyMh * assumptions.hourlyRateUsd,
        currentMh: null,
        currentUsd: null,
        savedMh: null,
        savedUsd: null,
        ...
        beforeUsd: legacyMh * assumptions.hourlyRateUsd,
        currentMh,
        currentUsd: currentMh * assumptions.hourlyRateUsd,
        savedMh: legacyMh - currentMh,
        savedUsd: (legacyMh - currentMh) * assumptions.hourlyRateUsd,
        segment: "current",
        fullSegment: isFullMonth,
      });
    ...
    const representativeRows = mean(monthlyRowCounts.filter((count) => count > 0));
    ...
    return finishResult(
      ...
      `${number(representativeRows, 1)} ${tr(lang, "records/month", "enregistrements/mois")}`,
      `${number(selected.length)} ${tr(lang, "total records", "enregistrements au total")} · ${money(assumptions.hourlyRateUsd, 2)}/${tr(lang, "MH", "HP")}`,
      {
  ...
  }, [analysisEnd, assumptions, lang, range.from, range.to, rows]);
  ...
  const controls = <>
    ...
    <div className="ti-controls assumptions">
      ...
      <NumberField label={tr(lang, "Labor cost · USD/MH", "Coût du travail · USD/HP")} value={assumptions.hourlyRateUsd} onChange={(value) => setAssumptions((current) => ({ ...current, hourlyRateUsd: value }))} />
    </div>
  </>;
...
function AutodeskProject({ lang, open, onToggle }: { lang: Lang; open: boolean; onToggle: (open: boolean) => void }) {
  ...
  const optimization = year === 2027 ? 0 : 43.2;
  const annualSavings = year === 2027 ? 0 : 24941;
  return (
    <details className="ti-project" open={open} onToggle={(event) => onToggle(event.currentTarget.open)}>
      <summary>
        ...
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Before · licenses", "Avant · licences")}</div><div className="ti-summary-value">{beforeLicenses}</div></div>
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "After · licenses", "Après · licences")}</div><div className="ti-summary-value">22</div></div>
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Optimization", "Optimisation")}</div><div className="ti-summary-value good">{percent(optimization)}</div></div>
        <div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Annualized USD savings", "Économies USD annualisées")}</div><div className="ti-summary-value">{money(annualSavings)}</div></div><div className="ti-chevron">⌄</div>
      </summary>
      <div className="ti-project-body">
        ...
        {year === 2027 ? <div className="ti-year-note"><strong>2027</strong><span>{tr(lang, "The optimized 22-license portfolio is the new baseline. No additional license optimization has been registered.", "Le portefeuille optimisé de 22 licences constitue la nouvelle référence. Aucune optimisation supplémentaire n’a été enregistrée.")}</span></div> : null}
        <div className="ti-kpis"><div className="ti-kpi gold"><div className="ti-label">{tr(lang, "Licenses before", "Licences avant")}</div><div className="ti-kpi-value">{beforeLicenses}</div></div><div className="ti-kpi cyan"><div className="ti-label">{tr(lang, "Licenses after", "Licences après")}</div><div className="ti-kpi-value">22</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "License reduction", "Réduction des licences")}</div><div className="ti-kpi-value">{year === 2027 ? 0 : 13}</div></div><div className="ti-kpi gold"><div className="ti-label">{tr(lang, "Annual cost before", "Coût annuel avant")}</div><div className="ti-kpi-value">{money(year === 2027 ? 32780 : 57721)}</div></div><div className="ti-kpi cyan"><div className="ti-label">{tr(lang, "Annual cost after", "Coût annuel après")}</div><div className="ti-kpi-value">{money(32780)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "Annual savings", "Économies annuelles")}</div><div className="ti-kpi-value">{money(annualSavings)}</div><div className="ti-sub">{percent(optimization)}</div></div></div>
        <div className="ti-method"><strong>{tr(lang, "Method.", "Méthode.")}</strong> {year === 2027 ? tr(lang, "The optimized 2026 license portfolio becomes the 2027 baseline: 22 → 22 licenses and USD 32,780 → USD 32,780 per year.", "Le portefeuille de licences optimisé en 2026 devient la référence 2027 : 22 → 22 licences et 32 780 USD → 32 780 USD par an.") : tr(lang, "Static comparison between the previous and optimized license scenarios: 35 → 22 licenses and USD 57,721 → USD 32,780 per year.", "Comparaison statique entre les scénarios de licences antérieur et optimisé : 35 → 22 licences et 57 721 USD → 32 780 USD par an.")}</div>
      </div>
    </details>
  );
}
...
function ProjectedProject({ definition, lang, open, onToggle, onResult, averageLots, averageCampaigns }: { definition: ProjectedDefinition; lang: Lang; open: boolean; onToggle: (open: boolean) => void; onResult: (key: string, metric: ProjectionMetric) => void; averageLots: number; averageCampaigns: number }) {
  ...
  const beforeMonthlyMh = before / 60 * volume;
  ...
  const displayedBeforeMinutes = year === 2027 ? after : before;
  const displayedSavedMinutes = year === 2027 ? 0 : savedMinutes;
  const displayedOptimization = year === 2027 ? 0 : optimization;
  const displayedMonthlyMh = year === 2027 ? 0 : monthlyMh;
  const displayedMonthlyUsd = year === 2027 ? 0 : monthlyUsd;
  const displayedAnnualUsd = year === 2027 ? 0 : metric.annualUsd;
  ...
  return (
    <details className="ti-project projected" open={open} onToggle={(event) => onToggle(event.currentTarget.open)}>
      <summary><div className="ti-project-title"><div className="ti-icon">P</div><div><div className="ti-project-name">{tr(lang, definition.titleEn, definition.titleFr)}</div><div className="ti-project-meta">{tr(lang, "Area", "Domaine")}: {tr(lang, definition.areaEn, definition.areaFr)} · Key User: {keyUser}</div><span className={`ti-status ${quantified ? "ok" : "bad"}`}>{year === 2027 ? tr(lang, "2027 baseline", "Référence 2027") : quantified ? tr(lang, "Projected assumptions", "Hypothèses projetées") : tr(lang, "Baseline pending", "Référence à définir")}</span></div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Before · MH/month", "Avant · HP/mois")}</div><div className="ti-summary-value">{quantified ? `${number(displayedBeforeMh, 2)} ${tr(lang, "MH/month", "HP/mois")}` : "—"}</div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "After · MH/month", "Après · HP/mois")}</div><div className="ti-summary-value">{quantified ? `${number(displayedAfterMh, 2)} ${tr(lang, "MH/month", "HP/mois")}` : "—"}</div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Optimization", "Optimisation")}</div><div className="ti-summary-value good">{percent(displayedOptimization)}</div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Annualized USD savings", "Économies USD annualisées")}</div><div className="ti-summary-value">{money(displayedAnnualUsd)}</div></div><div className="ti-chevron">⌄</div></summary>
      <div className="ti-project-body"><div className="ti-lead"><h3>{tr(lang, definition.titleEn, definition.titleFr)}</h3><p>{tr(lang, definition.descriptionEn, definition.descriptionFr)}</p></div><div className="ti-context"><div><span>{tr(lang, "Area", "Domaine")}</span><strong>{tr(lang, definition.areaEn, definition.areaFr)}</strong></div><div><span>Key User</span><strong>{keyUser}</strong></div><div className="description"><span>{tr(lang, "Optimization logic", "Logique d’optimisation")}</span><strong>{tr(lang, definition.logicEn, definition.logicFr)}</strong></div></div>
        {year === 2026 ? <div className="ti-controls assumptions"><NumberField label={`${tr(lang, "Before", "Avant")} · min-person/${unit}`} value={before} step={1} onChange={setBefore} /><NumberField label={`${tr(lang, "Projected", "Projeté")} · min-person/${unit}`} value={after} step={1} onChange={setAfter} /><ReadOnlyField label={`${tr(lang, "Assumed monthly volume", "Volume mensuel supposé")} · ${unit}`} value={volume} /><NumberField label={tr(lang, "Labor cost · USD/MH", "Coût du travail · USD/HP")} value={rate} onChange={setRate} /></div> : <div className="ti-year-note"><strong>2027</strong><span>{tr(lang, "The projected 2026 version is now the operating baseline. Before and after are equal.", "La version projetée de 2026 devient la référence opérationnelle. Les états avant et après sont identiques.")}</span></div>}
        <div className="ti-kpis"><div className="ti-kpi gold"><div className="ti-label">{tr(lang, "Before · min-person/unit", "Avant · min-personne/unité")}</div><div className="ti-kpi-value">{quantified ? number(displayedBeforeMinutes) : "—"}</div><div className="ti-sub">{unit}</div></div><div className="ti-kpi cyan"><div className="ti-label">{tr(lang, "After · min-person/unit", "Après · min-personne/unité")}</div><div className="ti-kpi-value">{quantified ? number(after) : "—"}</div><div className="ti-sub">{unit}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "Saved · min-person/unit", "Économie · min-personne/unité")}</div><div className="ti-kpi-value">{quantified ? number(displayedSavedMinutes) : "—"}</div><div className="ti-sub">{percent(displayedOptimization)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "MH saved / month", "HP économisées / mois")}</div><div className="ti-kpi-value">{number(displayedMonthlyMh, 2)}</div><div className="ti-sub">{number(volume)} {unit}/{tr(lang, "month", "mois")}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "USD saved / month", "USD économisés / mois")}</div><div className="ti-kpi-value">{money(displayedMonthlyUsd)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "USD saved / year", "USD économisés / an")}</div><div className="ti-kpi-value">{money(displayedAnnualUsd)}</div></div></div>
        <div className="ti-method"><strong>{tr(lang, "Method.", "Méthode.")}</strong> {year === 2027 ? tr(lang, "The optimized 2026 version is the new 2027 baseline. No additional improvement has been registered, so incremental optimization and savings are zero.", "La version optimisée de 2026 constitue la nouvelle référence 2027. Aucune amélioration supplémentaire n’ayant été enregistrée, l’optimisation et les économies incrémentales sont nulles.") : tr(lang, "Projected savings = (before − projected person-minutes) × monthly volume. Labor savings are converted to hours and valued using the editable hourly rate. Volume basis:", "Économies projetées = (minutes-personne avant − projetées) × volume mensuel. Les économies sont converties en heures et valorisées au coût horaire modifiable. Base de volume :")} {year === 2026 ? <b>{volumeBasis}</b> : null}{year === 2026 ? "." : null}</div>
      </div>
    </details>
  );
}
...
function MlProject({ rows, lang, open, onToggle, onResult }: { rows: MlRow[]; lang: Lang; open: boolean; onToggle: (open: boolean) => void; onResult: (metric: PotentialMetric) => void }) {
  ...
  const displayedSaved = year === 2027 ? 0 : avgSaved;
  const displayedTotalSaved = year === 2027 ? 0 : totalSaved;
  const displayedOptimization = year === 2027 ? 0 : optimization;
  const displayedAnnual = year === 2027 ? 0 : metric.annualUsd;
  const maxCost = Math.max(1, ...displayedMonthly.flatMap((row) => [row.actual, row.model]));
  const active = activeIndex === null ? null : displayedMonthly[activeIndex];
  return (
    <details className="ti-project potential" open={open} onToggle={(event) => onToggle(event.currentTarget.open)}><summary><div className="ti-project-title"><div className="ti-icon">ML</div><div><div className="ti-project-name">{tr(lang, "ML Refinery Consumption Optimization", "Optimisation ML de la consommation en raffinerie")}</div><div className="ti-project-meta">{tr(lang, "Area", "Domaine")}: {tr(lang, "Refinery", "Raffinerie")} · Key User: Richard Alcocer · GET /api/dti/ref-ml</div><span className={`ti-status ${rows.length ? "ok" : "bad"}`}>{year === 2027 ? tr(lang, "2027 baseline", "Référence 2027") : `${number(rows.length)} ${tr(lang, "records", "enregistrements")}`}</span></div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Before · USD/month", "Avant · USD/mois")}</div><div className="ti-summary-value">{money(displayedActual)}</div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "After · USD/month", "Après · USD/mois")}</div><div className="ti-summary-value">{money(avgModel)}</div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Optimization", "Optimisation")}</div><div className="ti-summary-value good">{percent(displayedOptimization)}</div></div><div className="ti-summary-metric"><div className="ti-label">{tr(lang, "Annualized USD savings", "Économies USD annualisées")}</div><div className="ti-summary-value">{money(displayedAnnual)}</div></div><div className="ti-chevron">⌄</div></summary>
      <div className="ti-project-body"><div className="ti-lead"><h3>{tr(lang, "ML Refinery Consumption Optimization", "Optimisation ML de la consommation en raffinerie")}</h3><p>{tr(lang, "A Machine Learning model estimates optimal input consumption for each refinery campaign, creating a data-driven benchmark to reduce material cost while preserving process requirements and operating performance.", "Un modèle de Machine Learning estime la consommation optimale d’intrants pour chaque campagne de raffinerie, créant une référence fondée sur les données afin de réduire le coût des matières tout en préservant les exigences du processus et la performance opérationnelle.")}</p></div><div className="ti-context"><div><span>{tr(lang, "Area", "Domaine")}</span><strong>{tr(lang, "Refinery", "Raffinerie")}</strong></div><div><span>Key User</span><strong>Richard Alcocer</strong></div><div className="description"><span>{tr(lang, "Calculation logic", "Logique de calcul")}</span><strong>{tr(lang, "Distinct campaign_id values are counted by campaign_date month. Costs are the sum of all campaign rows in each month; potential savings equal total actual consumption cost minus total ML-model consumption cost.", "Les campaign_id distincts sont comptés par mois de campaign_date. Les coûts correspondent à la somme de toutes les lignes de campagnes du mois ; l’économie potentielle est le coût réel total moins le coût total estimé par le modèle ML.")}</strong></div></div>{year === 2026 ? <DateFields {...range} lang={lang} /> : <div className="ti-year-note"><strong>2027</strong><span>{tr(lang, "The 2026 ML-model cost becomes the new consumption baseline. No incremental reduction has been recorded yet.", "Le coût du modèle ML de 2026 devient la nouvelle référence de consommation. Aucune réduction incrémentale n’a encore été enregistrée.")}</span></div>}
        <div className="ti-kpis"><div className="ti-kpi gold"><div className="ti-label">{tr(lang, "Baseline cost · USD/month", "Coût de référence · USD/mois")}</div><div className="ti-kpi-value">{money(displayedActual)}</div></div><div className="ti-kpi cyan"><div className="ti-label">{tr(lang, "Current cost · USD/month", "Coût actuel · USD/mois")}</div><div className="ti-kpi-value">{money(avgModel)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "Potential savings · USD/month", "Économies potentielles · USD/mois")}</div><div className="ti-kpi-value">{money(displayedSaved)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "Potential optimization", "Optimisation potentielle")}</div><div className="ti-kpi-value">{percent(displayedOptimization)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "Potential savings · selected period", "Économies potentielles · période")}</div><div className="ti-kpi-value">{money(displayedTotalSaved)}</div></div><div className="ti-kpi green"><div className="ti-label">{tr(lang, "Annualized potential", "Potentiel annualisé")}</div><div className="ti-kpi-value">{money(displayedAnnual)}</div></div></div>
        <div className="ti-chart-card ti-interactive-chart"><div className="ti-chart-title">{tr(lang, "Baseline vs. current consumption cost by month", "Coût de consommation de référence vs. actuel par mois")}</div><div className="ti-chart-sub">{tr(lang, "Gold = baseline cost · Blue = current cost", "Or = coût de référence · Bleu = coût actuel")}</div><div className="ti-chart-shell">{active ? <div className="ti-tooltip" style={{ left: `${Math.min(92, Math.max(8, ((Number(activeIndex) + .5) / displayedMonthly.length) * 100))}%` }}><strong>{active.month}</strong><span>{tr(lang, "Distinct campaigns", "Campagnes distinctes")}: <b>{active.campaigns}</b></span><span><i className="legacy" />{tr(lang, "Baseline cost", "Coût de référence")}: <b>{money(active.actual)}</b></span><span><i className="current" />{tr(lang, "Current cost", "Coût actuel")}: <b>{money(active.model)}</b></span><span>{tr(lang, "Potential savings", "Économies potentielles")}: <b>{money(active.saved)}</b></span></div> : null}<div className="ti-chart" onMouseLeave={() => setActiveIndex(null)}>{displayedMonthly.map((row, index) => <div className={`ti-chart-column ${activeIndex === index ? "active" : ""}`} key={row.month} tabIndex={0} aria-label={`${row.month}: ${money(row.actual)} / ${money(row.model)}`} onMouseEnter={() => setActiveIndex(index)} onFocus={() => setActiveIndex(index)} onBlur={() => setActiveIndex(null)}><div className="ti-bars"><span className="legacy" style={{ height: `${Math.max(2, row.actual / maxCost * 100)}%` }} /><span className="current" style={{ height: `${Math.max(2, row.model / maxCost * 100)}%` }} /></div><small>{row.month.slice(5)}</small></div>)}</div></div><div className="ti-chart-hint">{tr(lang, "Hover or focus a month to inspect campaigns and costs", "Survolez ou sélectionnez un mois pour afficher les campagnes et les coûts")}</div></div>
        <div className="ti-table-box"><div className="ti-table-head"><strong>{tr(lang, "Monthly campaign economics", "Économie mensuelle des campagnes")}</strong><span>{displayedMonthly.length} {tr(lang, "month(s)", "mois")}</span></div><div className="ti-table-wrap"><table><thead><tr><th>{tr(lang, "Month", "Mois")}</th><th>{tr(lang, "Distinct campaigns", "Campagnes distinctes")}</th><th>{tr(lang, "Baseline cost", "Coût de référence")}</th><th>{tr(lang, "Current cost", "Coût actuel")}</th><th>{tr(lang, "Potential savings", "Économies potentielles")}</th></tr></thead><tbody>{displayedMonthly.map((row) => <tr key={row.month}><td>{row.month}</td><td>{row.campaigns}</td><td>{money(row.actual)}</td><td>{money(row.model)}</td><td className="ti-total">{money(row.saved)}</td></tr>)}</tbody></table></div></div>
        <div className="ti-method"><strong>{tr(lang, "Method.", "Méthode.")}</strong> {year === 2027 ? tr(lang, "The optimized ML consumption level from 2026 is carried forward as both the baseline and current 2027 state. Incremental savings remain zero until a new improvement is measured.", "Le niveau de consommation ML optimisé de 2026 est reporté comme référence et état actuel de 2027. Les économies incrémentales restent nulles jusqu’à ce qu’une nouvelle amélioration soit mesurée.") : tr(lang, "Distinct campaigns are counted once per campaign_date month, while monthly actual and ML costs sum every campaign row in that month. Monthly potential savings are total consumption_cost_us minus total ml_consumption_cost_us; annualized potential is the average monthly difference × 12.", "Les campagnes distinctes sont comptées une fois par mois de campaign_date, tandis que les coûts réels et ML mensuels additionnent toutes les lignes de campagnes du mois. L’économie potentielle mensuelle est le total consumption_cost_us moins le total ml_consumption_cost_us ; le potentiel annualisé est l’écart mensuel moyen × 12.")}</div>
      </div></details>
  );
}
...
export default function TiPage() {
  ...
  const displayedExecutedAnnual = year === 2026 ? executedAnnual : 0;
  const displayedExecutedMh = year === 2026 ? portfolio.mh : 0;
  const displayedProjectedAnnual = year === 2026 ? projected.annual : 0;
    ...
    <div className="ti-dashboard">
      ...
      <main className="ti-main">
        ...
        <section className="ti-year-selector" aria-label={tr(lang, "Portfolio year", "Année du portefeuille")}>
          <div><span>{tr(lang, "Portfolio year", "Année du portefeuille")}</span><strong>{year}</strong><p>{year === 2026 ? tr(lang, "Transformation year · savings are measured against the legacy baseline.", "Année de transformation · les économies sont mesurées par rapport à la référence historique.") : tr(lang, "New baseline · the optimized 2026 state is carried forward. Incremental 2027 savings start at zero.", "Nouvelle référence · l’état optimisé de 2026 est reporté. Les économies incrémentales de 2027 commencent à zéro.")}</p></div>
          <label><span>{tr(lang, "Select year", "Sélectionner l’année")}</span><select value={year} onChange={(event) => setYear(Number(event.target.value) as PortfolioYear)}><option value={2026}>2026</option><option value={2027}>2027</option></select></label>
        </section>
      ...
      `}</style>
      <style jsx global>{`
        .ti-year-selector{display:flex;align-items:center;justify-content:space-between;gap:22px;margin-bottom:16px;padding:15px 18px;border:1px solid rgba(0,103,172,.22);border-left:5px solid var(--blue);border-radius:14px;background:linear-gradient(100deg,#fff 0%,#F4F9FC 100%);box-shadow:0 8px 22px rgba(20,47,66,.06)}.ti-year-selector>div>span,.ti-year-selector label>span{display:block;color:var(--muted);font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.ti-year-selector>div>strong{display:inline-block;margin-top:2px;color:var(--blue);font-size:27px}.ti-year-selector p{display:inline;margin:0 0 0 13px;color:#586A76;font-size:11px}.ti-year-selector label{display:flex;align-items:center;gap:9px;flex:none}.ti-year-selector select{min-width:112px;border:2px solid var(--blue);color:var(--blue);font-size:15px;font-weight:800;padding:9px 32px 9px 12px}.ti-year-note{display:flex;align-items:center;gap:12px;margin-bottom:14px;padding:11px 13px;border:1px solid rgba(0,103,172,.2);border-radius:10px;background:#F4F9FC;color:#586A76;font-size:10px;line-height:1.5}.ti-year-note strong{color:var(--blue);font-size:16px}.ti-year-note span{display:block}@media(max-width:700px){.ti-year-selector{align-items:flex-start;flex-direction:column}.ti-year-selector p{display:block;margin:4px 0 0}.ti-year-selector label{width:100%;justify-content:space-between}.ti-year-selector select{flex:0 0 120px}}
      `}</style>
    </div>
    </PortfolioYearContext.Provider>
  );
}
