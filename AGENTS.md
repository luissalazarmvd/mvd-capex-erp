...
- La columna `Ficha` permite marcar una o varias filas (incluye check en el encabezado para las filas visibles de cada tabla). Marcar una fila abre/focaliza su ficha; los botones Anterior/Siguiente y los COD marcados permiten navegar entre ellas. Los campos opcionales son siempre propios de cada activo: seleccionar varias filas no aplica valores masivamente ni sobrescribe datos.
