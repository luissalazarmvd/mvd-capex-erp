...
- Depreciación carga también el catálogo y `GET /api/actfij/mapping` para relacionar cada COD con su `origin_account_code`; los selectores dependientes Grupo y Denominación filtran por `account_group` y `account_denom` del mapping, junto con periodo, LR/DUP y búsqueda. Cambiar cualquiera limpia la selección.
- Al hacer clic en una fila de depreciación fuera de su checkbox y de sus celdas editables, se abre un preview superpuesto en la parte baja del área de trabajo (sin extender verticalmente la página ni reducir la tabla principal) con el histórico de ese COD: todos los periodos anteriores al año/mes seleccionado, con tasa, las cuatro variaciones de valor, los tres ajustes de depreciación, valores y saldos calculados. La fila enfocada queda sombreada en azul; repetir el clic en ella cierra el preview. Enfocar, editar o volver a hacer clic en una celda editable fuerza la apertura del preview, aun si ya corresponde a la fila activa, sin quitar el foco de la celda ni activar el toggle de la fila. El panel usa el mismo fondo, tamaño de letra y formato de tabla que la grilla principal.
...
- Solo el periodo calendario vigente de Lima es editable y seleccionable para envío; los periodos anteriores se muestran en modo consulta, sin inputs ni checks habilitados. Por ejemplo, el 24-08-2026 solo permite editar y guardar `2026-08`.
