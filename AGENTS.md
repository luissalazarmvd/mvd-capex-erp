...
- Marca interna VAi («Veta Analytics & Intelligence»): el logo animado es `VaiLogo` (`src/components/ui/VaiLogo.tsx`) y su secuencia vive en `.vai-logo-*` de `globals.css` —la V se dibuja como un check, el domo amanece sobre su horizonte, la estrella vuela y dibuja la A y la escarcha forma la i—; se reproduce una vez al montar y en bucle dentro de `.vai-data-loading-logo`. El portal `/` lo muestra con el lema en lugar del logo Veta Dorada y lee `next` en el servidor para que la cabecera llegue en el HTML sin reiniciar la animación al hidratar; las cabeceras de módulo conservan `logo_mvd.png`.
