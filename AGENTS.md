...
- Planta: `shift_date` pertenece al inicio de la guardia, incluido el turno nocturno. Usar los KPIs del balance; recuperaciones y ratios kg/TMS se recalculan desde sumas base; leyes ponderadas por TMS y humedad por TMH. Turno y supervisor son clasificadores. `shift_cost_usd` es el costo mensual contable prorrateado por guardia; el detalle por cuenta/CECO y USD/TMS (TMS del mes contada una vez) está en `plant_costs`; reactivos y bolas con precio logístico en `plant_consumables` (cantidades solo dentro del mismo insumo); leyes de carbón por tanque en `plant_carbon_tanks`; conciliación diaria contra Control de Mineral en `plant_cm_reconciliation` (solo meses cerrados).
- Refinería: `campaign_month` es el período de la campaña y `campaign_date` el inicio referencial; leyes Au/Ag se ponderan por carbón seco; stock es total a la fecha; cada insumo conserva su unidad y solo se suma consigo mismo, los costos USD sí se consolidan. El costo de insumos (cantidad × precio unitario logístico vigente; GLP con factor kg→gal) está disponible por campaña (`refinery_campaigns`) y por insumo/subproceso con el consumo óptimo ML (`refinery_consumption`); costos por gramo de Au o kg de carbón cuentan el valor de campaña una sola vez.
