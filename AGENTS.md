...
- UI compartida en `src/components/ui`: `Button`, `Input`, `Select` (nativo), `Dropdown` (botón + menú propio), `Table`, `Pager`, `TopNav`, `FastCellInput`, `DateInput`, `ExcelHeaderFilter`/`ExcelFilters`; estilos globales en `src/app/globals.css`. Un control que se repite en dos módulos va ahí, no como copia local.
...
- Carga de datos: el overlay `.vai-data-loading` (tarjeta con `VaiLogo` en bucle sobre fondo oscuro) es la única señal de «cargando desde endpoints» del ERP. `DataLoading` (`src/components/ui/DataLoading.tsx`, montado en `src/app/layout.tsx`) lo muestra en todos los módulos a partir de las lecturas en curso que registra `apiGet` en `src/lib/apiClient.ts` (`subscribeInflight`/`getInflightPaths`); solo cuentan los GET, no `apiPost` ni `apiDownload`, y aparece tras 150 ms para no parpadear. Un `apiGet(path, { silent: true })` no lo dispara: reservado para consultas ligadas al tecleo (lookups de RUC y licencia en Kardex Guías). Quedan fuera `/vai`, que tiene su propio overlay con el nombre de cada fuente, y `/ti`. Los módulos conservan sus `disabled`/textos locales de carga; no añadir spinners propios. Todo filtro de fecha que dispare una consulta usa `DateInput` (`src/components/ui/DateInput.tsx`, prop `onCommit`), no `<input type="date">` con `onChange`: el nativo emite `change` en cada flecha o dígito sobre un segmento y recargaba a cada paso; `DateInput` confirma al elegir un día en el popup, con Enter o al salir del campo (Escape descarta). Hoy lo usan Trazabilidad (Ingresar, Estado, Contabilidad, CM Inputs), Activos Fijos · Historial y Planta · Guardia; las fechas que solo filtran en cliente o son celdas editables siguen con el input nativo.
