...
- Datos reales: el navegador consulta cada fuente por su `endpoint` del catálogo vía `apiClient`; la sección `ENGINE` de `src/lib/vai.ts` filtra y agrega en cliente reutilizando los gráficos de `src/components/ui/Charts.tsx`. Filtros, «Actualizar datos», abrir, renombrar y eliminar no llaman a OpenAI.
