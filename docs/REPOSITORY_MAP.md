...
| Trazabilidad | `src/app/traceability/upload`, `entries`, `status`, `conta`, `cm-inputs` | `src/components/traceability/*`; CM Inputs en `TraceabilityCmInputsForm.tsx`, validación autoritativa de su guardado en `src/app/traceability/cm-inputs/actions.ts` | `/api/traceability/*` y contratos localizados desde el formulario correspondiente |
