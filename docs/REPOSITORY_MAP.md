...
| V-Ai | `src/app/vai` | `src/components/vai/VaiWorkspace.tsx` (orquestador), `VaiPromptForm.tsx`, `VaiDashboard.tsx` (renderer), `VaiInventory.tsx`; catálogo, spec/validación, motor y OpenAI en `src/lib/vai/*`; guard de scope en `src/lib/auth/session.ts` | `POST /api/vai/generate` (route local, única llamada a OpenAI); persistencia externa `/api/vai/dashboards*`; datos por los GET declarados en `src/lib/vai/catalog.ts` |
